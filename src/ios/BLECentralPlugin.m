...
- (void)pluginInitialize {
    ...
    bluetoothStates = [NSDictionary dictionaryWithObjectsAndKeys:
                       ...
                       nil];
    autoDisconnectOnBackground = [NSMutableDictionary new];
    inBackground = [NSMutableDictionary new];
...
- (void)connect:(CDVInvokedUrlCommand *)command {
    ...
    NSNumber *boolParam = [command argumentAtIndex:1];
    
    autoDisconnectOnBackground[@"automaticDisconnect"] = boolParam;
    autoDisconnectOnBackground[@"deviceUUID"] = uuid;
...
- (void)startScanWithOptions:(CDVInvokedUrlCommand*)command {
    ...
    discoverPeripheralCallbackId = [command.callbackId copy];
    NSArray<NSString *> *serviceUUIDStrings = [command argumentAtIndex:0];
    NSNumber *scanInBackground = [command argumentAtIndex:2];
    NSArray<CBUUID *> *serviceUUIDs = [self uuidStringsToCBUUIDs:serviceUUIDStrings];
    ...
    bool inBackgroundAndShouldNotScan = [scanInBackground isEqual: @(NO)] && [inBackground[@"status"] isEqual: @(YES)];
    if (!inBackgroundAndShouldNotScan) {
...
{
    inBackground[@"status"] = @(YES);
    if([autoDisconnectOnBackground[@"automaticDisconnect"] isEqual: @(YES)]) {
        CBPeripheral *peripheral = [self findPeripheralByUUID:autoDisconnectOnBackground[@"deviceUUID"]];
        
        if (!peripheral) {
            NSString *message = [NSString stringWithFormat:@"Peripheral %@ not found", autoDisconnectOnBackground[@"deviceUUID"]];
            NSLog(@"%@", message);
        } else {
